...
local ALIGN = require 'misc.ALIGNMENT'
...
function layer:__init(opt)
  ...
  self.LMtype = utils.getopt(opt, 'LMtype', 'encoder')
  local dropout = utils.getopt(opt, 'dropout', 0)
  -- options for Language Model
  self.seq_length = utils.getopt(opt, 'seq_length')
  self.context = utils.getopt(opt, 'context')
  self.batch_size = utils.getopt(opt, 'batch_size')
  self.num_encoders = utils.getopt(opt, 'num_encoders', 1)
  -- create the core lstm network. note +1 for both the START and END tokens
  if self.LMtype == 'decoder' then
    self.align = ALIGN.alignment(self.batch_size, self.num_encoders*(self.seq_length+1), self.rnn_size )
  end
  self.core = LSTM.lstm(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout, self.context)
...
function layer:setModelName(name)
  self.name = name
end

function layer:getModelName()
  return self.name
end

function layer:setLMtype(category)
  self.LMtype = category
end

function layer:getLMtype()
  return self.LMtype
end
...
function layer:createClones()
  ...
  if self.LMtype == 'decoder' then 
    ...
    for t=2,self.seq_length+2 do
      self.aligns[t] = self.align:clone('weight', 'bias', 'gradWeight', 'gradBias')
    end
  end
end
...
function layer:getModulesList()
  if self.LMtype == 'decoder' then
    return {self.core, self.lookup_table, self.align}    
  else
...
function layer:parameters()
  ...
  if self.LMtype == 'decoder' then
    local p3,g3 = self.align:parameters()
...
function layer:training()
  ...
  if self.LMtype == 'decoder' then 
    for k,v in pairs(self.aligns) do v:training() end
  end
end
...
function layer:evaluate()
  ...
  if self.LMtype == 'decoder' then 
    for k,v in pairs(self.aligns) do v:evaluate() end
  end
end
...
--]]
function layer:sample(imgs, opt, annotaions)
  ...
  if self.LMtype == 'encoder' then 
    h_states = torch.FloatTensor(self.seq_length+1, batch_size, self.rnn_size):cuda()
  end
  if self.LMtype == 'decoder' then 
    att_result = torch.FloatTensor(self.seq_length+1, batch_size, 99)
  end
  ...
  for t=1,self.seq_length+2 do
    ...
    if self.LMtype == 'decoder' then
      local ct
      local prob
      if t>1 and not can_skip then
        local annotation_batch_size = self.num_encoders*(self.seq_length+1)
        local extended_s = torch.zeros(self.batch_size*annotation_batch_size, self.rnn_size):cuda()
        local prev_s = state[2]
        for i = 1,self.batch_size do 
          extended_s[{ {(i-1)*annotation_batch_size+1,i*annotation_batch_size}, {} }] = prev_s[{ {i}, {} }]:expand(annotation_batch_size, self.rnn_size)
        end
          --print (extended_s)
        ct = self.aligns[t]:forward{annotaions, extended_s}
        prob = self.aligns[t]:get(11).output --probbility of each encoders hiddenstates
        att_result[t-1] = prob:float()
        --table.insert(att_result, prob)
      else
        ct = torch.zeros(self.batch_size, self.rnn_size)
      end
      --[[
      -- equal weights version
      if t>1 and not can_skip then
        ct = self.aligns[t]:forward(annotaions)
        print (t, ct[{ 38, {300,305} }])
      else
        ct = torch.zeros(self.batch_size, self.rnn_size)
      end
      ]]
      table.insert(inputs, ct:cuda())
    end
    ...
    if self.LMtype == 'encoder' and t > 1 then 
      h_states[t-1] = out[2]:clone() -- record the current hidden states
    end
  end
  -- return the samples and their log likelihoods
  if self.LMtype == 'encoder' then 
    return seq, seqLogprobs, h_states
  else
    return seq, att_result, seqLogprobs
  end
end
...
function layer:updateOutput(input)
  --print ("rnn vocab_size:", self.vocab_size)
  local imgs = input[1]
  local seq = input[2]
  local h_states
  if self.LMtype == 'decoder' then 
  ...
  assert(seq:size(1) == self.seq_length)   -- if load model to fintune, the new dataset must has the same max_length with the old model
  ...
  for t=1,self.seq_length+2 do
    ...
    elseif t == 2 then
      -- @@@@@@@@ alignment model first forward here.......
      -- feed in the start tokens
      ...
      local it = seq[t-2]:clone()  --whole batch in same time stamp 
    ...
    if self.LMtype == 'decoder' then
      ...
      if t>1 and not can_skip then
        local annotation_batch_size = self.num_encoders*(self.seq_length+1)
        local extended_s = torch.zeros(batch_size*annotation_batch_size, self.rnn_size):cuda()
        local prev_s = self.state[t-1][2]
        for i = 1,batch_size do 
          extended_s[{ {(i-1)*annotation_batch_size+1,i*annotation_batch_size}, {} }] = prev_s[{ {i}, {} }]:expand(annotation_batch_size, self.rnn_size)
        end
          --print (extended_s)
        self.aligns_inputs[t] = {h_states, extended_s}
        ct = self.aligns[t]:forward(self.aligns_inputs[t])
      else
        ct = torch.zeros(batch_size, self.rnn_size)
      end
      --[[
      -- equal weights version
      if t>1 and not can_skip then
    ...
    if not can_skip then
      ...
      table.insert(self.inputs[t], ct:cuda())
      -- forward the network
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    --print ('core', torch.typename(self.inputs[t][1]), torch.typename(self.inputs[t][2]), torch.typename(self.inputs[t][3]), torch.typename(self.inputs[t][4]))
    ...
    if self.LMtype == 'decoder' and t>1 then
      local dct = dinputs[4]
      self.aligns[t]:backward(self.aligns_inputs[t], dct)
    end
...
function crit:updateOutput(input, label)
  ...
  for t = 2,L do
    --print ('time:', t-1)
    for b = 1,N,5 do
      --print ('batch start at:', b)
      local target_b_idx = 0
      local target_v_idx = 0
      if t-1 > D then -- we are out of bounds of the index sequence: pad with null tokens
        target_index = 0
      else
        local min_logprob = 10000
        for offset = 0,4 do
          --print (b+offset)
          candidate_target_b_idx = b+offset
          candidate_target_v_idx = seq[ {t-1, candidate_target_b_idx} ]
          --print ('b_idx', candidate_target_b_idx, 'v_idx', candidate_target_v_idx )  
          if candidate_target_v_idx ~= 0 then
            candidate_logprob = input[ {t, candidate_target_b_idx, candidate_target_v_idx} ]
            if -candidate_logprob < min_logprob then
              --print ('compare:', -candidate_logprob, min_logprob)
              min_logprob = -candidate_logprob
              target_b_idx = candidate_target_b_idx
              target_v_idx = candidate_target_v_idx
            end
          end
        end
      end
      -- the first time we see null token as next index, actually want the model to predict the END token
      if target_v_idx == 0 and t==2 then
        target_v_idx = Mp1
      end

      -- if there is a non-null next token, enforce loss!
      if target_v_idx ~= 0 then
        -- accumulate loss
        --print ('b_idx', target_b_idx, 'v_idx', target_v_idx, 'score', score[ target_b_idx ] )  
        loss = loss - input[{t, target_b_idx, target_v_idx}]*score[ target_b_idx ] -- log(p)
        self.gradInput[{t, target_b_idx, target_v_idx }] = -1*score[ target_b_idx ]
        n = n + 1
      end      
    end
  end
  ...
  for b=1,N do -- iterate over batches
    ...
    for t=2,L do -- iterate over sequence time (ignore t=1, dummy forward for the image)
      ...
      if t-1 > D then -- we are out of bounds of the index sequence: pad with null tokens
        ...
        target_index = 0
        local best_logprob = 0
        local p = (b-1)%5
        print ('batch:', b)
        for i= b+(0-p), b+(4-p) do
          local candidate_target_idx = seq[ {t-1, i} ]
          if (candidate_target_idx ~= 0) then
            local candidate_logprob= input[ { t,b,candidate_target_idx } ] 
            print ('idx', candidate_target_idx, 'prob', candidate_logprob, 'best', best_logprob)
            if candidate_logprob < best_logprob then
              target_index = candidate_target_idx
      ...
      if target_index ~= 0 then
        -- accumulate loss
        loss = loss - input[{ t,b,target_index }]*score[b] -- log(p)
        self.gradInput[{ t,b,target_index }] = -1*score[b]
        n = n + 1
      end
