...
function layer:__init(opt)
  ...
  if self.LMtype == 'decoder' then
    self.attention = attention.setup(self.batch_size, self.num_encoders*(self.seq_length+1), self.rnn_size )
  end
...
function layer:getModulesList()
  if self.LMtype == 'decoder' then
    return {self.core, self.lookup_table, self.attention}    
  else
...
function layer:parameters()
  ...
  if self.LMtype == 'decoder' then
    local p3,g3 = self.attention:parameters()
...
function layer:sample(imgs, opt, annotaions)
  ...
  for t=1,self.seq_length+2 do
    ...
    elseif t == 2 then
      ...
      if sample_max == 1 then
        ...
        if temperature == 1.0 then
          ...
          prob_prev = torch.exp(torch.div(logprobs, temperature)),m
        end
    ...
    if self.LMtype == 'decoder' then
      ...
      if t>1 and not can_skip then
        ...
        local prev_s = state[self.num_state]
        for i = 1,self.batch_size do 
          extended_s[{ {(i-1)*annotation_batch_size+1,i*annotation_batch_size}, {} }] = prev_s[{ {i}, {} }]:expand(annotation_batch_size, self.rnn_size)
        end
        st = self.attentions[t]:forward{annotaions, extended_s}  -- attention calculation is here
        prob = self.attentions[t]:get(11).output --probability of each encoders hiddenstates
        att_result[t-1] = prob:float()
        --table.insert(att_result, prob)
      else
        st = torch.zeros(self.batch_size, self.rnn_size)
      end
      table.insert(inputs, st:cuda())
    end

    local out = self.core:forward(inputs) -- LSTM calculation is here
    logprobs = out[self.num_state+1] -- last element is the output vector
    state = {}
    for i=1,self.num_state do table.insert(state, out[i]) end
    if self.LMtype == 'encoder' and t > 1 then 
      h_states[t-1] = out[self.num_state]:clone() -- record the current hidden states
    end
  end
  ...
  if self.LMtype == 'encoder' then 
    ...
    return seq, seqLogprobs, att_result
  end
end
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+2 do
    ...
    if self.LMtype == 'decoder' then
      ...
      if t>1 and not can_skip then
        ...
        for i = 1,batch_size do 
          ...
          --print (extended_s)
        self.attentions_inputs[t] = {h_states, extended_s}
        st = self.attentions[t]:forward(self.attentions_inputs[t])
      else
        st = torch.zeros(batch_size, self.rnn_size)
      end
    end
    ...
    if not can_skip then
      ...
      table.insert(self.inputs[t], st:cuda())
      -- forward the network
      local out = self.clones[t]:forward(self.inputs[t]) -- LSTM calculation
      -- process the outputs
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    if self.LMtype == 'decoder' and t>1 then
      local dst = dinputs[4]
      self.attentions[t]:backward(self.attentions_inputs[t], dst)
    end
...
function crit:updateOutput(input, seq)
  ...
  local L,N,Mp1 = input:size(1), input:size(2), input:size(3)  L: sentence_length, N: batch_size, Mpl: vocab_size
  local D = seq:size(1)
