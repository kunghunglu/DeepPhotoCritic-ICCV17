...
function DataLoader:getBatch(opt)
  ...
  local score_batch = torch.LongTensor(batch_size * seq_per_img)
  ...
  for i=1,batch_size do
    ...
    if ncap < seq_per_img then
      ...
      score = self.h5_file:read('/scores'):partial( {ixl, ixl+seq_per_img-1})
    end
    local il = (i-1)*seq_per_img+1
    label_batch[{ {il,il+seq_per_img-1} }] = seq
    score_batch[{ {il,il+seq_per_img-1} }] = score
