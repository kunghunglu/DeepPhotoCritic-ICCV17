...
function net_utils.language_eval(predictions, id)
  ...
  utils.write_json('coco-caption/val_' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  print ('./misc/call_python_caption_eval.sh val_' .. id .. '.json')
  os.execute('./misc/call_python_caption_eval.sh val_' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('coco-caption/val_' .. id .. '.json') -- god forgive me
