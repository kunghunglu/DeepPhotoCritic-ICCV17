...
function DataLoaderRaw:__init(opt)
  local index_json = utils.getopt(opt, 'index_json', '')
  ...
  self.files = {}
  self.ids = {}
  if string.len(opt.index_json) > 0 then
    print('reading from ' .. opt.index_json)
    -- read in filenames from the coco-style json file
    self.coco_annotation = utils.read_json(opt.index_json)
    --print (self.coco_annotation.images)
...
function DataLoaderRaw:getBatch(opt)
  ...
  for i=1,batch_size do
    ...
    table.insert(names, paths.basename(self.files[ri]))
